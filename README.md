...
Ver.1.0 試作12 修正版2では、修正版1でも復旧画面に止まる問題に対応し、独立した `boot.js` で起動段階の診断、復旧ボタン、安全モードを本体初期化から分離しました。試作12のたまご育成、鳥の仲間、なかまずかん、スタート画面の仕様は維持しています。

## バージョン
- 表示名: 結羽ちゃんの冒険
- バージョン: Ver.1.0 試作12 修正版2
- appVersion: `1.0.0-prototype.12-hotfix.2`
- schemaVersion: `1`
- キャッシュ対策: `?v=10p12h2`
...
- 画像パス: `./apple-touch-icon.png?v=10p12h2`
- 最低表示時間: 1.2秒
- 通常終了目安: 初期化完了後
- フェイルセーフ: 約4秒
- フェードアウト後にDOMから除去
...
## 技術条件
外部API、CDN、fetch、module形式のscript、画像生成AI、外部画像、外部SVG、素材ダウンロードは使用していません。鳥の仲間とたまごの見た目はHTML、CSS、JavaScript、手書きインラインSVGで実装しています。
