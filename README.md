...
Ver.1.0 試作16では、iPhoneのホーム画面からSafariではなく独立Webアプリとして開けるstandalone対応を正式版へ統一し、くじゃくの尾羽と本体比率を調整しました。たまご育成、なかまずかん、親モードのぬりえ設定、起動診断、safeStartモードは維持しています。
...
- `companion_peacock` の `designVersion` を3へ更新
...
## 試作15の鳥原画修正
鳥の仲間6種類は、画像生成・外部画像・外部SVGを使わず、既存のインラインSVG定義を手修正しました。試作16では、くじゃくだけ追加修正しています。
...
- 画像パス: `./apple-touch-icon.png?v=10p16`
- 最低表示時間: 1.2秒
- 通常終了目安: 初期化完了後
- フェイルセーフ: 約4秒
- フェードアウト後にDOMから除去
...
- manifest: `./manifest.webmanifest?v=10p16`
