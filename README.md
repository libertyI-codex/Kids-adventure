...
Ver.1.0 試作11では、採用済み画像をそのまま使用してiPhoneホーム画面用アイコンを追加しました。

## バージョン
- 表示名: 結羽ちゃんの冒険
- 内部プロジェクト名: こどもの冒険
- バージョン: Ver.1.0 試作11
- appVersion: `1.0.0-prototype.11`
- schemaVersion: `1`
...
## iPhoneホーム画面用アイコン
採用済み画像 `assets/icons/yuu-bouken-icon-adopted.png` を、画像処理せず同一バイト列のまま `apple-touch-icon.png` として配置しています。

`index.html` のhead内で次を指定しています。

```html
<link rel="apple-touch-icon" href="./apple-touch-icon.png?v=10p11">
```

今回対応したのはiPhoneホーム画面用アイコンのみです。Android用アイコン、favicon、Web App Manifest、マスク可能アイコン、SNS用OGP画像は追加・変更していません。iPhone側で角丸マスクが適用されるため、CSSや画像加工による追加の角丸、白枠、影、トリミングは行っていません。
...
## 世界
Ver.1.0 試作11時点では、次の6世界をすべて初期利用可能にしています。
...
試作11でも保存キーは変更していません。schemaVersionも `1` のままです。
