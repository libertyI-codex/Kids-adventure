...
Ver.1.0 試作22では、ひよこ本体を包んでいた外周ストロークを削除し、ぺんぎん、しまえなが、いんこ、ぶんちょう、オリジナルの「こおりの でんせつどり」を追加しました。既存6種と合わせて11種が、従来の固定seed・未取得優先ルールでたまごから生まれます。
...
- `companion_ice_legend_bird`（こおりの でんせつどり）を、既存IPを複製しないオリジナルデザインで追加
...
既存の取得済み鳥、孵化履歴、お気に入り、`hatchCount`、`mealCount`、`bondLevel` は変更しません。同じ鳥が再び生まれた場合も、従来どおりカードは増やさず `hatchCount` と `bondLevel` を増やします。JSONバックアップは従来のappData形式を維持し、保存キーとschemaVersionは変更していません。
...
- 画像パス: `./apple-touch-icon.png?v=10p21h1`
- 最低表示時間: 1.2秒
- 通常終了目安: 初期化完了後
- フェイルセーフ: 約4秒
- フェードアウト後にDOMから除去
...
- manifest: `./manifest.webmanifest?v=10p21h1`
