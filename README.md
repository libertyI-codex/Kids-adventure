...
Ver.1.0 試作21 修正1では、ひよこ・オウム・ふくろうのSVGルートに透明背景と外枠なしを明示し、一部の表示環境で鳥の周囲に現れる不要な黒い四角い枠を取り除きました。鳥本体の輪郭、目、くちばし、羽、足、模様、viewBoxと表示倍率は変更していません。試作21のおでかけを含む既存機能と保存データはそのまま維持しています。

既存のホーム画面アイコンは古いHTML設定を保持するため、一度削除してからSafariで最新版を開き、改めてホーム画面へ追加してください。

## バージョン
- 表示名: 結羽ちゃんの冒険
- バージョン: Ver.1.0 試作21 修正1
- appVersion: `1.0.0-prototype.21-hotfix1`
- schemaVersion: `1`
- キャッシュ対策: `?v=10p21h1`
